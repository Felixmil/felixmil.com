...
subtitle: "First day of the advent of code using R"
date: 2022-12-01
...
> with 7000, 8000, and 9000 Calories, a total of 24000 Calories. The
> fifth Elf is carrying one food item with 10000 Calories. In case the
> Elves get hungry and need extra snacks, they need to know which Elf to
> ask: they'd like to know how many Calories are being carried by the
...
for (line in input) {
  if (line == "") { # if empty line, then it is a new elf*
    # previous elf is saved
    result_df <- rbind(result_df, 
                       tibble(elf_id = elf_id, 
                     total_calories = calories))
    # elf_id is increase since we move to next elf
    elf_id <- elf_id + 1
    # calories counter is reset
    calories <- 0
  } else { # Le line is not empty so it's just more calories brough by this elf
    calories <- calories + as.numeric(line)
  }
}
```
...
```{r}
arrange(result_df, desc(total_calories))
```

```{r, echo=FALSE}
result <-
  result_df %>%
  top_n(1, total_calories)
```

The elf carrying the most calories is the **`r result$elf_id`th** whom carries **`r format(result$total_calories, scientific = F)` calories**.
...
top_3 <-
  result_df %>%
  top_n(3, total_calories)

top_3
```


The Three elves carrying the most calories are the **`r paste(top_3$elf_id, collapse=", ")`**th with a total of **`r format(sum(top_3$total_calories),scientific=FALSE, big.mark = " ")` calories**.